...
public class Main {
    ...
    public static void calculatorWithX(String[] strings) {
        ...
        for (String string : strings) {
            if (string.equals("=") && countEquals == 0) {
                ...
                System.out.println("Ошибка: нельзя присваивать значение нескольким переменным одновременно");
                return;
            }
        }
    ...
    public static void printX(String str) {
        switch (str.strip()) {
            ...
            default -> System.out.println("Ошибка: Несуществующая переменная");
        }
    }
    ...
    private static void xValue(String x, BigInteger value) {
        switch (x.strip()) {
            ...
            case "x4" -> x4 = value;
            case "x5" -> x5 = value;
            default -> System.out.println("Ошибка: Неверное название переменной");
        }
    }
            ...
            if (isNumber(strings[i])) {
                ...
                if (countOperators >= countNumbers) {
                    ...
                    if (i > 0 && (isOperator(strings[i-1]) || isOperator(strings[i+1]))){
                        System.out.println("Ошибка: Слишком много арифметических операторов");
                    } else {
                ...
                System.out.println("Ошибка: Нельзя вводить отрицательные числа");
                error = false;
                break;
                ...
                System.out.println("Ошибка: Некорректное присваивание значения переменной");
                error = false;
                break;
            }else {
                System.out.println("Ошибка: Неизвестный символ, возможно Вы ввели несуществующую переменную");
                error = false;
                break;
            }
        }
